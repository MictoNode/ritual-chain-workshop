...
/** datetime-local value = now + N hours, in the input's expected format. */
...
export function CreateBountyForm({ onCreated }: { onCreated?: (bountyId: bigint) => void }) {
  ...
  const subTs = useMemo(() => new Date(submissionDeadline).getTime(), [submissionDeadline]);
  const revealTs = useMemo(() => new Date(revealDeadline).getTime(), [revealDeadline]);
  ...
  const validation = useMemo(() => {
    ...
    if (revealTs <= subTs) return "Reveal deadline must be after the submission deadline.";
    if (reward !== "") {
      try {
        parseEther(reward);
      } catch {
    ...
    return null;
  }, [title, rubric, submissionDeadline, revealDeadline, reward, subTs, revealTs]);
      ...
      <CardBody>
        ...
        <form onSubmit={handleSubmit} className="mt-3 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Field label="Submission deadline" hint="Commitments accepted until this time.">
              <Input
                type="datetime-local"
            ...
            <Field label="Reveal deadline" hint="Answers are revealed in this window, then judged.">
              <Input
                type="datetime-local"
